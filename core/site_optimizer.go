...
func (site *Site) optimizerUpdate(battery []types.Measurement) error {
	...
	if minLen < expectedSlots {
		if solarTariff != nil {
			return fmt.Errorf("not enough slots for meaningful optimization: %d < %d (grid=%d, feedIn=%d, solar=%d)", minLen, expectedSlots, len(grid), len(feedIn), len(solar))
		}
		return fmt.Errorf("not enough slots for meaningful optimization: %d < %d (grid=%d, feedIn=%d)", minLen, expectedSlots, len(grid), len(feedIn))
	}
